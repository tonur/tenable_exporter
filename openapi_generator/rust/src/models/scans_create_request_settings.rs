...
pub struct ScansCreateRequestSettings {
    ...
    /// For one-time scans, the starting time and date for the scan. For recurrent scans, the first date on which the scan schedule is active and the time that recurring scans launch based on the `rrules` parameter.  This parameter must be in the following format: `YYYYMMDDTHHMMSS`.   This parameter corresponds to the **Starts** drop-down boxes in scan schedule settings in the Tenable.io user interface.
    ...
    /// The timezone of the scheduled start time for the scan (as specified in the `starttime` parameter). Use the [GET /scans/timezones](ref:scans-timezones) endpoint to determine this value.   Corresponds to the **Timezone** drop-down box in the scan schedule settings in the Tenable.io user interface.
    #[serde(rename = "timezone", skip_serializing_if = "Option::is_none")]
    pub timezone: Option<String>,
    /// The list of targets to scan. For a full list of supported target formats, see the [Tenable Vulnerability Management User Guide](https://docs.tenable.com/vulnerability-management/Content/Scans/AboutScanTargets.htm). You can specify multiple targets (of differing formats) as a comma-delimited list.   This parameter is required if your request omits other target parameters. For more information, see \"Required Scan Target Parameters\" in [Create a Scan](doc:create-scan-tio).   **Note:** Tenable.io does not perform validation on values you submit for this parameter. If you submit invalid values, Tenable.io stores the invalid values in the scan configuration, and when the scan runs, scanning fails on the invalid targets.
    ...
    /// Creates a unique identifier on hosts scanned using credentials.
    #[serde(rename = "host_tagging", skip_serializing_if = "Option::is_none")]
    pub host_tagging: Option<String>,
...
impl ScansCreateRequestSettings {
    pub fn new(name: String, enabled: bool) -> ScansCreateRequestSettings {
        ScansCreateRequestSettings {
            name,
            description: None,
            policy_id: None,
            folder_id: None,
